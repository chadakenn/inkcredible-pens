...
export default function ImageMigrationPanel({ onCatalogChanged }: { onCatalogChanged: () => void }) {
  ...
  return <section className="mb-5 rounded-xl border border-cyan/30 bg-ink-2 p-5">
    ...
    <div className="mt-4 rounded-xl bg-ink p-4"><div className="flex justify-between text-sm font-bold"><span className="text-cream">{data?.remaining ?? '…'} external photos remaining</span><span className="text-cyan">{running ? `${progress}%` : data?.status?.status?.replaceAll('_', ' ') || 'Ready'}</span></div><div className="mt-2 h-3 overflow-hidden rounded-full bg-ink-3"><div className="h-full bg-lime transition-all" style={{ width: `${progress}%` }} /></div>{data?.status && <p className="mt-2 text-xs text-mute">Migrated {data.status.migrated} · Failed {data.status.failed} · Skipped {data.status.skipped}</p>}</div>
