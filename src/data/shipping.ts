/** Standard shipping is $8 (free at $60); carts with canvas or large prints ship for $15. */
...
export function shippingCentsForSubtotal(subtotalCents: number, hasCanvas = false, hasLargePrint = false): number {
...
export function shippingDollarsForSubtotal(subtotalDollars: number, hasCanvas = false, hasLargePrint = false): number {
  return shippingCentsForSubtotal(Math.round(Number(subtotalDollars) * 100), hasCanvas, hasLargePrint) / 100
}
