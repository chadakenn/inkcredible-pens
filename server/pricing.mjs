...
function priceLargePrint(config) {
  ...
  return {
    ...
    custom: {
      ...
      ...(Number(config?.artworkPixelWidth) > 0 ? { artworkPixelWidth: Math.round(Number(config.artworkPixelWidth)) } : {}),
      ...(Number(config?.artworkPixelHeight) > 0 ? { artworkPixelHeight: Math.round(Number(config.artworkPixelHeight)) } : {}),
      ...(Number(config?.estimatedPrintDpi) > 0 ? { estimatedPrintDpi: Math.round(Number(config.estimatedPrintDpi)) } : {}),
      ...(['vector', 'good', 'caution', 'blurry'].includes(config?.artworkQuality) ? { artworkQuality: config.artworkQuality } : {}),
      ...(config?.artworkUrl ? { artworkUrl: config.artworkUrl } : {}),
      ...(config?.artworkId ? { artworkId: config.artworkId } : {}),
